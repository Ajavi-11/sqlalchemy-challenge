...
def precipitation():
    ...
    print("Recieved precipitation api request.")

    final_date_query = session.query(func.max(func.strftime("%Y-%m-%d"))).all()
...
def stations():
    ...
    print("Recieved station api request.")
...
def tobs():
    """Return a JSON of temperature observations for the previous year."""

    print("Received tobs api request.")

    # Query dates and temp observations of most active station of the last year of data
...
def start(start):
    ...
    print("Recieved start date api request.")

    # last date in database
    final_date_query = session.query(func.max(func.strftime("%Y-%m-%d", Measurement.date))).all()
    max_date = final_date_query[0][1]
...
def start_end(start, end):
    """Return a JSON list of the minimum temperature, the average temperature, and the maximum temperature for a given start of start-end range."""
